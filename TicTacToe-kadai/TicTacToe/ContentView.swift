...
struct ContentView: View {
    ...
    static func showAIAlert(won: SquareStatus, reset: @escaping () -> Void) -> Alert {
        return Alert(title: Text("ゲームオーバー"),
              message: Text(won != .empty ? won == .x ? "勝った!" : "負けた!" : "引き分け!"),
              dismissButton: Alert.Button.destructive(Text("オッケー"), action: reset)
        )
    }
    
    static func showPVPAlert(won: SquareStatus, reset: ()) -> Alert {
        Alert(title: Text("ゲームオーバー"),
              message: Text(won != .empty ? won == .x ? "X 勝ち!" : "O 勝ち!" : "引き分け!"),
              dismissButton: Alert.Button.destructive(Text("オッケー"), action: { reset }
        ))
    }
                    ...
                    ZStack { // 4
                        VStack{
                            ...
                            Text("TicTacToe")
                                ...
                                .padding(.top, 20)
                                .foregroundColor(colorScheme == .dark ? Color.white.opacity(0.7) : Color.black.opacity(0.7))
                            Text("TikTacToe, made by Ducar")
                                .bold()
                                .font(.title3)
//                                .padding(.top)
                                .foregroundColor(colorScheme == .dark ? Color.white.opacity(0.7) : Color.black.opacity(0.7))
//                                .padding(.horizontal, 80)
                            Link("GitHub Page", destination: URL(string: "https://github.com/ducarQQ")!)
                                .font(.headline)
                                .padding(.top, 3)
                            Spacer(minLength: 5)
                            Image("PreviewImage")
            ...
            Button(action: {
                ...
                if self.selection == false {
                    if viewModel.winner == .x { text = "勝った!" }
                    else if viewModel.winner == .o { text = "負けた!" }
                    else { text = "引き分け!" }
                } else {
                    if viewModel.winner == .x { text = "X 勝ち!" }
                    else if viewModel.winner == .o { text = "O 勝ち!" }
                    else { text = "引き分け!" }
                }
